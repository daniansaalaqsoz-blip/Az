...
cpu_core_manager() {
    ...
    while true; do
        clear
        echo "===== CPU CORE MANAGER ====="
        echo "Jumlah core: $CORES (0-$((CORES-1)))"
        echo
        echo "[1] All ON (hidupin semua core)"
        echo "[2] Only little (cpu0-3)"
        echo "[3] Only big (cpu4-7)"
        echo "[4] Custom (pilih core sendiri)"
        echo "[0] Kembali"
        echo
        read -p "Pilih: " c
        ...
        case $c in
        1)
            echo "[+] Menghidupkan semua core..."
            for i in $(seq 0 $((CORES-1))); do
                su -c "echo 1 > /sys/devices/system/cpu/cpu$i/online"
            done
            echo "✅ Semua core hidup"
            sleep 1
            ;;
        2)
            echo "[+] Only little cores ON (cpu0-3)"
            for i in $(seq 0 $((CORES-1))); do
                if [ $i -le 3 ]; then
                    su -c "echo 1 > /sys/devices/system/cpu/cpu$i/online"
                else
                    su -c "echo 0 > /sys/devices/system/cpu/cpu$i/online"
                fi
            done
            echo "✅ Little cores aktif"
            sleep 1
            ;;
        3)
            echo "[+] Only big cores ON (cpu4-7)"
            for i in $(seq 0 $((CORES-1))); do
                if [ $i -ge 4 ]; then
                    su -c "echo 1 > /sys/devices/system/cpu/cpu$i/online"
                else
            ...
            echo "✅ Big cores aktif"
            sleep 1
            ;;
        4)
            echo "Masukkan core yang mau hidup (pisah spasi, contoh: 0 2 5 7):"
            read -a CUSTOM
            for i in $(seq 0 $((CORES-1))); do
                if [[ " ${CUSTOM[@]} " =~ " $i " ]]; then
                    su -c "echo 1 > /sys/devices/system/cpu/cpu$i/online"
